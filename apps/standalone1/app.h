...
private:
    ...
    void draw(const Wall& wall, wxBufferedPaintDC& dc, wxColour color);
    bool ValidateInputs();
...
void MyFrame::StartSimulation() {
    // Get user input
    int numMovers = numMoversCtrl->GetValue();
    int maxSpeed = maxSpeedCtrl->GetValue();
    int seed = seedCtrl->GetValue();
    float minMass = 5;
    float maxMass = 1000;//= maxMassCtrl->GetValue();
    ...
    float springStrength = std::stof(softCollideParamSpringCtrl->GetValue().ToStdString());
    float repulsionStrength = std::stof(softCollideParamRepulsionCtrl->GetValue().ToStdString());
    float coulombK = std::stof(coulombParamKCtrl->GetValue().ToStdString());
    bool gravityEnabled = gravityCheckbox->IsChecked();
    bool springEnabled = springCheckbox->IsChecked();
    bool softCollideEnabled = softCollideCheckbox->IsChecked();
    bool coulombEnabled = coulombCheckbox->IsChecked();
    

    // Update frame title
    SetTitle("Simulation Running");

    // Here, replace this with your simulation logic, e.g., creating a new panel or canvas.
    simulationPanel = new wxPanel(this);
    wxBoxSizer* simulationSizer = new wxBoxSizer(wxVERTICAL);

    // wxStaticText* infoText = new wxStaticText(simulationPanel, wxID_ANY, 
    //     wxString::Format("Simulation started with %d movers and max speed %d.", numMovers, maxSpeed));
    // simulationSizer->Add(infoText, 0, wxALL | wxALIGN_CENTER, 20);

    simulationPanel->SetSizer(simulationSizer);
    simulationPanel->Show();
    //add button to stop simulation in top right corner
   ...
   //enable interactions
    ...
    //add wall
    // simulator.add_wall(Vect2(-X/2, -Y/2), Vect2(-X/2, Y/2));
    // simulator.add_wall(Vect2(X/2, -Y/2), Vect2(X/2, Y/2));
    // simulator.add_wall(Vect2(-X/2, -Y/2), Vect2(X/2, -Y/2));
    // simulator.add_wall(Vect2(-X/2, Y/2), Vect2(X/2, Y/2));
    
    //add force fields that kind of act as springy walls
    // auto leftField = std::function<Vect2(Vect2)>([](Vect2 pos) {
    //     if (pos.x < 0) return Vect2(-1000*pos.x, 0);
    //     else return Vect2();
    // });
    // auto rightField = std::function<Vect2(Vect2)>([X](Vect2 pos) {
    //     if (pos.x > X) return Vect2(-1000*(pos.x-X), 0);
    //     else return Vect2();
    // });
    // simulator.add_effect(new ForceField(leftField));
    // simulator.add_effect(new ForceField(rightField));

    //add movers
    int moverCount = numMovers;
    int max_speed = maxSpeed;
    int maxCharge = 10;
    std::srand(seed);
    for (int i = 0; i < moverCount; i++) {

        Vect2 pos = Vect2(randRange(0, X), randRange(0, Y));
        Vect2 vel = Vect2();
        vel.polar_set(randRange(0, max_speed), randRange(0, 360));
    ...
    // simulator.add_mover(typeid(NewtMover), MoverArgs(Vect2(50,300), Vect2(0,0))); 
...
void MyFrame::OnPaint(wxPaintEvent& event) {
    ...
    // draw the walls
    for (auto& wall : simulator.walls) {
        draw(*wall, dc, wxColour(255, 255, 255));
    }
...
void MyFrame::draw(const Wall& wall, wxBufferedPaintDC& dc, wxColor color) {
    dc.SetBrush(color);
    //set pen to not transparent
    dc.SetPen(*wxWHITE_PEN);
    dc.DrawLine(wall.pointA.x, wall.pointA.y, wall.pointB.x, wall.pointB.y);
}
